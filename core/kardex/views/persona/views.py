...
class PersonaListView(PermissionMixin, ListView, LoginRequiredMixin):
    model = Persona
...
class PersonaCreateView(PermissionMixin, LoginRequiredMixin, CreateView):
    model = Persona
...
class PersonaUpdateView(PermissionMixin, UpdateView, LoginRequiredMixin):
    model = Persona
    form_class = PersonaForm
    template_name = 'persona/create.html'
    success_url = reverse_lazy('kardex:persona_list')
...
class PersonaDeleteView(PermissionMixin, LoginRequiredMixin, DeleteView):
    model = Persona
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['title'] = 'Eliminación de personal'
        context['entity'] = 'Persona'
        context['list_url'] = self.success_url
        return context

class PersonaDetailView(PermissionMixin, LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['bienes'] = Bienes.objects.filter(resguardatario_id = pk)
        return context
